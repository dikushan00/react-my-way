...
var news_reducer = function (state, action) {
    if (state === void 0) { state = initialState; }
    switch (action.type) {
        case 'SN/News/ADD_NEW_COMMENT': {
            if (action.newCommentText.length > 0) {
                return __assign(__assign({}, state), { news: state.news.map(function (item) {
                        if (item.id === action.id)
                            item.comments = __spreadArrays(item.comments, [{ userId: 5, fullName: 'dikushan00', commentId: 5, text: action.newCommentText, photo: 'https://cdn.freelance.ru/img/portfolio/pics/00/3A/2C/3812566.jpg?mt=d239b33b' }]);
                        return item;
                    }) });
                id++;
            }
        ...
        case 'SN/News/VIEW_STORY': {
            return __assign(__assign({}, state), { stories: state.stories.map(function (item) {
                    if (item.id === action.id)
                        item.viewed = true;
                    return item;
                }) });
...
exports.actions = {
    addNewComment: function (id, newCommentText) { return ({ type: 'SN/News/ADD_NEW_COMMENT', id: id, newCommentText: newCommentText }); },
    updateLikeCount: function (id) { return ({ type: 'SN/News/UPDATE_LIKE_COUNT', id: id }); },
    updateCommentCount: function (id) { return ({ type: 'SN/News/UPDATE_COMMENT_COUNT', id: id }); },
    viewStory: function (id) { return ({ type: 'SN/News/VIEW_STORY', id: id }); }
};
exports["default"] = news_reducer;
